...
**状态流转**: `待设计方案` → `待安保方案设计` → `待备案申请` → `备案材料已交接` → `审批通过`（GovLiaison批准，系统自动→审批通过-待举办，原手动确认步骤已移除）→ `审批通过-待举办` → `举办中`（自动）→ `已结束`（AdminStaff标记）；另有 `待补充备案材料` `不通过/已终止` `已取消` `已延期`（12 状态，详见 `docs/state-machine.md`）
