# 活动状态机

活动从立项到审批结束的完整生命周期。状态变迁由 UC1-UC6 驱动，箭头标注了触发用例和条件。

```mermaid
stateDiagram-v2
    [*] --> 待设计方案 : UC1 立项成功\n(场地/时间校验通过)

    待设计方案 --> 待安保方案设计 : UC2 最终确定方案\n(至少一个版本生成后点击最终确定)
    待设计方案 --> 已取消 : UC6 强制变更(不可抗力)
    待设计方案 --> 已延期 : UC6 强制变更(不可抗力)

    待安保方案设计 --> 待安保方案设计 : UC3 负责人驳回\n(编制人员重新修改)
    待安保方案设计 --> 待备案申请 : UC3 签署完成\n(风险评估表+责任书签章)
    待安保方案设计 --> 已取消 : UC6 强制变更
    待安保方案设计 --> 已延期 : UC6 强制变更

    待备案申请 --> 备案材料已交接 : UC4 打包+线下交接\n(签名齐全, 材料合规)
    待备案申请 --> 已取消 : UC6 强制变更
    待备案申请 --> 已延期 : UC6 强制变更

    备案材料已交接 --> 审批通过-待举办 : UC5 政府通过\n(上传批文, 标注通过)
    备案材料已交接 --> 待补充备案材料 : UC5 需补充材料\n(政府要求补件)
    备案材料已交接 --> 不通过/已终止 : UC5 政府驳回\n(上传驳回通知书)
    备案材料已交接 --> 已取消 : UC6 强制变更
    备案材料已交接 --> 已延期 : UC6 强制变更

    待补充备案材料 --> 备案材料已交接 : 安保部补充后重新递交
    待补充备案材料 --> 已取消 : UC6 强制变更
    待补充备案材料 --> 已延期 : UC6 强制变更

    审批通过-待举办 --> 举办中 : 到达预计举办时间\n(系统自动转换)
    审批通过-待举办 --> 已取消 : UC6 强制变更(不可抗力)
    审批通过-待举办 --> 已延期 : UC6 强制变更(不可抗力)

    举办中 --> 已结束 : AdminStaff 标记活动结束
    举办中 --> 已取消 : UC6 强制变更
    举办中 --> 已延期 : UC6 强制变更

    已结束 --> [*] : 活动正常结束

    不通过/已终止 --> [*] : 流程终止

    已取消 --> [*]
    已延期 --> [*]
```

## 状态说明

| 状态            | 含义                                   | 进入条件           |
| --------------- | -------------------------------------- | ------------------ |
| 待设计方案      | 立项完成，等待宣策部编制方案（可生成多版，点击最终确定进入下一阶段） | UC1 保存立项       |
| 待安保方案设计  | 方案已最终确定，等待安保部编制安保预案 | UC2 最终确定方案   |
| 待备案申请      | 安保方案签署完成，等待打包备案         | UC3 电子签名完成   |
| 备案材料已交接  | 纸质材料已提交给政府对接人员           | UC4 线下交接确认   |
| 审批通过-待举办 | GovLiaison 审批通过后系统直接进入，等待举办时间到达 | UC5 GovLiaison 批准 |
| 举办中          | 活动正在举办                           | 系统自动（到达预计时间） |
| 已结束          | 活动正常结束                           | AdminStaff 标记结束 |
| 待补充备案材料  | 政府要求补充材料，安保部修改后重新打包交接       | UC5 政府要求补件   |
| 不通过/已终止   | 政府驳回，活动终止                     | UC5 政府驳回       |
| 已取消          | 因不可抗力等原因强制取消，后续操作锁定 | UC6 行政部强制变更 |
| 已延期          | 因不可抗力等原因强制延期，后续操作锁定 | UC6 行政部强制变更 |

### 安保方案审核状态 (`SecurityPlan.audit_status`)

| 状态   | 触发                                       |
| ------ | ------------------------------------------ |
| 待编制 | 活动进入「待安保方案设计」，SecurityPlan 创建 |
| 待签署 | SecurityOfficer 完成编制，提交备案申请     |
| 已签署 | SecurityManager 签署确认                   |
| 已审核 | 政府审批通过（→ 审批通过-待举办）            |

```mermaid
stateDiagram-v2
    [*] --> 待编制 : 进入待安保方案设计
    待编制 --> 待签署 : SecurityOfficer 完成编制
    待编制 --> 待编制 : 驳回回环
    待签署 --> 已签署 : SecurityManager 签署
    待签署 --> 待编制 : 驳回回环
    已签署 --> 已审核 : 政府审批通过
```

### UC2 最终确定方案（活动方案）

Promoter 在活动方案 tab 点击"最终确定方案"→ 系统校验字段完整性 → 不通过弹窗列出问题 → 通过则弹出确认弹窗 → 确认后流转到「待安保方案设计」。已替代 WorkflowActions 的"提交到安保方案设计"按钮。

### UC3 提交审核 + 签署（安保方案 + 双表 + 承诺书）

1. SecurityOfficer 在安保方案 tab 内切换三个子 tab：安保方案 | 风险评估表 | 责任确认书
2. 每个子 tab 独立填写 →"提交生成"（**延迟类型仅保存数据快照，不生成 DOCX**）→ 可选生成多版本
3. 双表字段支持跨实体自动填入：activity_start/end/content/crowd_scale 从活动方案预填，project_name/activity_type/location_type 从 Activity 预填，固定值从 .env 注入
4. 安保方案子 tab 点击"提交审核"→ 系统校验字段完整性 → 通过则 `audit_status` = "待签署"，通知 SecurityManager
5. SecurityManager 进入安保方案 tab → 看到方案内容 + 签名上传区 → 可"确认签署"或"驳回"
6. **签署第一步**：Manager 上传签名 → 系统一次性生成含 Manager 签名的安保方案 + 双表 DOCX → 备案承诺书签署区出现
7. **签署第二步**：Manager 确认备案承诺书（全部字段 autofill，复用已上传签名）→ 生成承诺书 DOCX → 流转到「待备案申请」
8. 驳回：Manager 勾选预设原因 + 补充说明 → `audit_status` 回到"待编制" → 通知 Officer → 红色横幅 + 高亮字段 → 驳回后**必须先创建新版本**才能重新提交（防重复提交）
9. 签署后 Officer 看到绿色"已签署确认"横幅 + "前往备案材料打包"按钮

## 关键规则

- **已结束/已取消/已延期/不通过已终止**是终态，进入后所有后续操作锁定
- 审批通过-待举办到达 estimated_time 后，系统在查询时自动转为举办中
- **待补充备案材料**不是终态，安保部补充后可重新递交
- 逾期检测（UC2 备选流 2b）不改变状态，仅触发预警通知
- **安保方案 DOCX 延迟生成**：SecurityOfficer 提交生成时仅保存数据快照（minio_path=NULL），Manager 签署后一次性生成含签名 DOCX
- **活动方案最终确定后锁定**：进入待安保方案设计后，Promoter 只能查看活动方案，不能编辑或生成新版本
- **角色视图分离**：活动方案 → Promoter 编辑/AdminStaff 管理版本/其他只读；安保方案 → SecurityOfficer 编辑/SecurityManager 签署+管理版本/AdminStaff 管理版本/其他只读
- **UC5 GovLiaison 审查**：逐条审查全部 5 项材料（合格/不合格 + 意见）→ 全部材料审查完毕后可"审批通过"或"要求补件"，驳回不受限。审批通过后系统自动流转至「审批通过-待举办」并通知所有经手过活动的人员。审批决策生成 ApprovalRecord 记录。
- **UC5b 补件回路**：待补充备案材料 → Officer 修改材料 → 重新生成版本 → 重新打包 → 重新交接 → 流转回「备案材料已交接」→ 通知 GovLiaison 重新审查。复用 UC4 打包/交接逻辑。
- **系统自动流转**：审批通过-待举办到达 estimated_time 后，路由层在查询时调用 WorkflowService.transition(SYSTEM) 转为「举办中」。
- **提交审核全字段校验**：三表（安保方案+双表）所有可见字段必须填写，弹窗列出缺失项，点击跳转到对应子 tab 并高亮
