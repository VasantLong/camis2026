...
活动从立项到审批结束的完整生命周期。状态变迁由 UC1-UC6 驱动，箭头标注了触发用例和条件。
...
stateDiagram-v2
    ...
    待设计方案 --> 已取消 : UC6 强制变更(不可抗力)
    待设计方案 --> 已延期 : UC6 强制变更(不可抗力)
    ...
    审批通过 --> 审批通过-待举办 : 系统自动流转\n(原手动确认步骤已移除)
    审批通过 --> 已取消 : UC6 强制变更
    审批通过 --> 已延期 : UC6 强制变更

    审批通过-待举办 --> 举办中 : 到达预计举办时间\n(系统自动转换)
    审批通过-待举办 --> 已取消 : UC6 强制变更(不可抗力)
    审批通过-待举办 --> 已延期 : UC6 强制变更(不可抗力)
...
| 审批通过        | Liaison 已批准（历史中间态，v0.29 起跳过） | UC5 上传批文(通过) |
| 审批通过-待举办 | Liaison 审批通过后系统自动进入，等待举办时间到达 | UC5 GovLiaison 批准 |
| 举办中          | 活动正在举办                           | 系统自动（到达预计时间） |
| 已结束          | 活动正常结束                           | AdminStaff 标记结束 |
| 待补充备案材料  | 政府要求补充材料，安保部修改后重新打包交接       | UC5 政府要求补件   |
| 不通过/已终止   | 政府驳回，活动终止                     | UC5 政府驳回       |
| 已取消          | 因不可抗力等原因强制取消，后续操作锁定 | UC6 行政部强制变更 |
| 已延期          | 因不可抗力等原因强制延期，后续操作锁定 | UC6 行政部强制变更 |
...
- **原手动确认步骤已移除**：GovLiaison 审批通过后系统自动流转至「审批通过-待举办」，不再需要 Manager 确认。审批通过时通知所有经手过活动的人员（owner, designer, manager, 历史操作人）。
