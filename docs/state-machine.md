...
| 状态 | 含义 | 进入条件 |
|------|------|----------|
| 待设计方案 | 立项完成，等待宣策部编制方案 | UC1 保存立项 |
| 待安保方案设计 | 方案已提交，等待安保部编制安保预案 | UC2 方案提交 |
| 待备案申请 | 安保方案签署完成，等待打包备案 | UC3 电子签名完成 |
| 备案材料已交接 | 纸质材料已提交给政府对接人员 | UC4 线下交接确认 |
| 审批通过 | 政府已批准，等待安保部最终确认 | UC5 上传批文(通过) |
| 审批通过-待举办 | 全流程闭环，活动可合法举办 | UC6 安保部确认通过 |
...
- **已取消/已延期**是终态，进入后所有后续操作锁定（不可再提交方案、备案、审批）
- **UC6 安保部驳回**是逆向流转：审批通过的批文被打回，回到"待安保方案设计"重新出具方案（风险最高）
