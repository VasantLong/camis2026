...
> **此处插入界面截图（图3-5 创建立项界面）**。Promoter 填写活动名称、类型、预计时间、地点、主办方信息、截止时间后保存立项。系统实时校验场地冲突，冲突时弹出阻断提示。

> **此处插入界面截图（图3-6 角色感知导航）**。左侧为 Promoter 侧边栏（我的活动、新建立项），右侧为 GovLiaison 侧边栏（待审查材料、审批记录）。不同角色看到不同的菜单入口，无权限入口直接隐藏。

> **此处插入界面截图（图3-7 活动详情页全景）**。活动详情页为多 Tab 聚合页，顶部展示活动基本信息和当前状态，下方五个 Tab（详情/文档/活动方案/安保方案/备案）按角色和状态分段渲染各自的操作界面。

> **此处插入界面截图（图3-8 Schema 驱动动态表单）**。左侧为活动方案表单（TemplateForm，Promoter 编辑中），右侧为安保方案表单（SecurityOfficer 编辑中，高风险模式下显示全部条件字段）。字段类型包括文本、日期（含时刻）、下拉选择、多行文本、动态列表、签名上传。

> **此处插入界面截图（图3-9 安保方案三子 Tab）**。安保方案 Tab 内含三个子 Tab（安保方案/风险评估报备表/责任确认书），风险等级为高风险时显示医疗保障方案、消防方案、人群控制方案等条件字段。

> **此处插入界面截图（图3-10 备案 Tab — 打包与交接）**。SecurityOfficer 视角：5 项材料签署状态列表（含活动方案、安保方案、风险评估报备表、责任确认书、备案承诺书），全部签署后"打包备案材料"按钮可用，打包完成后显示"下载打包文件"和"确认纸质交接"。

> **此处插入界面截图（图3-11 备案 Tab — 政府审查面板）**。GovLiaison 视角：材料列表 + 逐条合格/不合格标记 + 意见输入框 + 批量操作按钮。全部审查完毕后，审批决策区提供"审批通过"（需上传批文）、"要求补充材料"、"驳回"三个选项。

> **此处插入界面截图（图3-12 Manager 两步签署）**。第一步：Manager 上传签名后确认签署，系统生成含签名的安保方案+双表 DOCX。第二步：备案承诺书签署区出现（全字段 autofill），Manager 复用已上传签名完成承诺书签署。

> **此处插入界面截图（图3-13 工作台 Dashboard）**。顶部统计卡片（总活动数、审批通过率、本月新增），中部各状态分布进度条，底部异常活动列表。右上角月报导出按钮。

> **此处插入界面截图（图3-14 月报导出渲染界面）**。选择月份后系统异步生成 PDF 月报，生成完成后通过通知铃铛推送下载链接。月报 PDF 包含活动统计图表、审批概况、异常活动清单。

> **此处插入界面截图（图3-15 超级管理员用户管理界面）**。用户列表（含角色、状态、最后登录时间），支持角色编辑、启用/禁用、归档操作。右侧角色申请审批面板。
